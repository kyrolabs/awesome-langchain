...
- [Lumin](https://github.com/amitbidlan/zistica-lumin): Open-source platform for running LLM agents in production — **observe, govern, defend, operate** in a single self-hosted Docker container. Full-trace observability, policy engine with shadow/enforce modes + suggester + replay + drift detection, OWASP LLM Top 10 runtime guardrails (Presidio NER, Prompt Guard 2, Llama Guard 4), 12 starter policy packs (OWASP, GDPR, HIPAA, PCI-DSS), human approvals queue, audit trail, tenant-isolation firewall, webhooks. 16 framework integrations — LangChain auto-instrumentation via `LUMIN_TRACING=true`. Apache-2.0. ![GitHub Repo stars](https://img.shields.io/github/stars/amitbidlan/zistica-lumin?style=social)
