...
- [Lumin](https://github.com/amitbidlan/zistica-lumin): Open-source local-first observability + security platform for LangChain agents. Full-trace recording (LLM calls, tools, retrievals, costs, evals), OWASP LLM Top 10 protections (prompt-injection guard, sensitive-info redaction with Presidio NER, insecure-output filter, agent autonomy controls), policy engine, and a tenant-isolation firewall for multi-tenant bots. Zero-config auto-instrumentation via `LUMIN_TRACING=true`. Self-hosted single Docker container, DuckDB-backed, Apache-2.0. ![GitHub Repo stars](https://img.shields.io/github/stars/amitbidlan/zistica-lumin?style=social)
