...
- [semantic-llm-cache](https://github.com/karthyick/prompt-cache): Decorator-based semantic LLM response caching — wrap any LLM call with `@semantic_cache` to skip 20-40% of API calls when prompts are semantically similar (provider-agnostic, lightweight alternative to GPTCache for single-process apps) ![GitHub Repo stars](https://img.shields.io/github/stars/karthyick/prompt-cache?style=social)
- [langgraph-crosschain](https://github.com/karthyick/langgraph-crosschain): Cross-chain node communication for LangGraph multi-agent systems — enables direct calls between nodes in different graphs (`Chain1.NodeA → Chain2.NodeB`) plus shared state and message broadcasting across chains ![GitHub Repo stars](https://img.shields.io/github/stars/karthyick/langgraph-crosschain?style=social)
